...
Public Class CtlPlanFormuExterna
    ...
    Private MsjObs As String = ""
    Private XmlNroFormulaObs As String
...
#Region "EditarFilaGrilla-Actualizar"
    ...
    Private Sub EditarFormulacionExterna(ByVal dr As DataGridViewRow)
        If dr.Cells("entidad").Value.ToString() <> "" Then
        ...
        'If dr.Cells("MedControl").Value.ToString = "S" Then
        '    Me.LblCantidad.Text = "Cantidad Dispensar"
        'Else
        '    Me.lblCantidad.Text = "Cantidad Total"
        'END If
        'Me.LblCantidad.Refresh()
    ...
    Private Sub dgFormulacion_UserDeletingRow(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowCancelEventArgs) Handles dgFormulacion.UserDeletingRow
        ...
        'If IsDBNull(e.Row.Cells("entidad").Value) Then

        'Else
        '    If EliminarMedicamento(e.Row.Cells("nro_formula").Value, XmlManual(e.Row.Cells("producto").Value.ToString, "Medicamento Consulta Externa")) = False Then
        '        e.Cancel = True
        '        Exit Sub
        '    End If
        'End If
        ...
        If e.Row.Cells("nro_formula").Value > 0 Then
            ...
            Else
                ...
                '    If EliminarMedicamento(e.Row.Cells("nro_formula").Value, XmlManual(e.Row.Cells("producto").Value.ToString, MsjObs)) = False Then
                '        e.Cancel = True
                '        Exit Sub
                '    End If
                'End If

                Dim rspSiNo As DialogResult = MsgBox("¿Está seguro de suspender un medicamento conciliado?", MsgBoxStyle.YesNo)
                If rspSiNo = MsgBoxResult.No Then
                    e.Cancel = True
                    Exit Sub
                End If

                strInfo = InputBox("Motivo de suspensión del medicamento", "Observaciones", MsjObs)
                MsjObs = Trim(strInfo)
                If MsjObs.ToString <> "" Then
                    If MsjObs.Trim.Length < 10 Or MsjObs.Trim.Length > 200 Then
                        e.Cancel = True
                        MsgBox("La observación debe contener entre 10 a 200 caracteres.")
                        Exit Sub
                    Else
                        If EliminarMedicamento(e.Row.Cells("nro_formula").Value, XmlManual(e.Row.Cells("producto").Value.ToString, MsjObs)) = False Then
                            e.Cancel = True
...
#Region "Grabar"
    Private Sub btGrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btGrabar.Click
        ...
        For Each Fila As DataGridViewRow In dgFormulacion.Rows
            If IsDBNull(Fila.Cells("entidad").Value) Then
                MessageBox.Show("Existen Antecedentes farmacológicos pendientes por conciliar.", "Historia Clínica", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
        Next
...
#End Region
        ...
        If ValidarDatos() Then
            If strModificar = "" Then
                ...
                If VerificarMedicamento(IIf(objFormuExt.EsNuevaFormula = True, 0, objFormuExt.NumeroFormulaActual)) Then
                    ...
                    dtRow("cantidadDispensacion") = IIf(Me.tbCantidadPrescripcion.Text = "", DBNull.Value, CDbl(Me.tbCantidadPrescripcion.Text))
            ...
            Else
                ...
                dgFormulacion.CurrentRow.Cells("cantidadDispensacion").Value = CDbl(Trim(tbCantidadPrescripcion.Text))
                dgFormulacion.CurrentRow.Cells("Cantidad").Value = CDbl(Trim(txtDosis.Text))
                ' Else
                ...
                If IsDBNull(dgFormulacion.CurrentRow.Cells("entidad").Value) Then
                    dgFormulacion.CurrentRow.Cells("adicionado").Value = "S"
                End If
                dgFormulacion.CurrentRow.Cells("pais").Value = objGeneral.Pais
                dgFormulacion.CurrentRow.Cells("departamen").Value = objGeneral.Departamento
                dgFormulacion.CurrentRow.Cells("ciudad").Value = objGeneral.Ciudad
                dgFormulacion.CurrentRow.Cells("entidad").Value = tbCodEntidad.Text
                dgFormulacion.CurrentRow.Cells("des_entidad").Value = cbEntidad.Text
                dgFormulacion.CurrentRow.Cells("diastrat").Value = Me.txtDiasTrat.Text
                ...
                dgFormulacion.CurrentRow.Cells("UniMedDosis").Value = IIf(Me.txtDescripcionUM.SelectedValue = "", DBNull.Value, Me.txtDescripcionUM.SelectedValue)
                ...
                dgFormulacion.CurrentRow.Cells("CantTotalLetras").Value = IIf(lblletras.Text = "", DBNull.Value, Val(Me.tbCantidadPrescripcion.Text) & " (" & lblletras.Text & ") " & lblUnidadPrescripcion.Text)
                'End If
    ...
    Private Sub dgFormulacion_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles dgFormulacion.CellFormatting
        For Each Fila As DataGridViewRow In dgFormulacion.Rows
            If IsDBNull(Fila.Cells("entidad").Value) Then
                Fila.DefaultCellStyle.BackColor = Color.Khaki
            Else
    ...
    Private Function XmlManual(ByVal strMedicamento As String, ByVal strObservacion As String) As String
        Dim strEnc As String = "<?xml version=""1.0"" encoding=""ISO-8859-1"" ?><Raiz>"
        Dim strCur As String
        strCur = strEnc + "<Row strMedicamento=""" + strMedicamento + """ obs=""" + strObservacion + """ login= """ + objGeneral.Login + """/></Raiz>"
        Return strCur
    End Function
End Class
